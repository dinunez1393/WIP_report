...
def df_splitter(dataframe, category_name='SerialNumber'):
    """
    Function splits a dataframe by category into four partitions (23%-28%-24%-25%)
    ...
    partition_sizes = (int(len(unique_categories) * 0.23), int(len(unique_categories) * 0.51),
                       int(len(unique_categories) * 0.75))
